...
template <typename T>
MaxHeap2<T>::MaxHeap2(vector<T>* tree) {
    this->tree = tree;
...
template <typename T>
void MaxHeap2<T>::swap(T& a, T& b) {
    int tmp; tmp = a; a = b; b = tmp;
}

template <typename T>
void MaxHeap2<T>::buildMaxHeap() {
    for(int i = heapSize/2; i >= 0; i--)
        maxHeapify(i);
}

template <typename T>
void MaxHeap2<T>::maxHeapify(int i) {
    int l = left(i);
    int r = right(i);
    int max = i;
...
template <typename T>
void MaxHeap2<T>::inOrderAscii(int i, int spazio) {
    if(i >= heapSize)
        return;
    spazio += vuoto;
...
template <typename T>
void MaxHeap2<T>::printAsciiTree() {
