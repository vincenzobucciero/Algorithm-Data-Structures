...
template <typename T>
MinHeap2<T>::MinHeap2(vector<T>* tree) {
    this->tree = tree;
...
template <typename T>
void MinHeap2<T>::swap(T& a, T& b) {
    int tmp; tmp = a; a = b; b = tmp;
}

template <typename T>
void MinHeap2<T>::buildMinHeap() {
    for(int i = heapSize/2; i >= 0; i--)
        minHeapify(i);
}

template <typename T>
void MinHeap2<T>::minHeapify(int i) {
    int l = left(i);
    int r = right(i);
    int min = i;
...
template <typename T>
void MinHeap2<T>::inOrderAscii(int i, int spazio) {
    if(i >= heapSize)
        return;
    spazio += vuoto;
...
template <typename T>
void MinHeap2<T>::printAsciiTree() {
