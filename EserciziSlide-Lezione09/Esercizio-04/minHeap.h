...
#define vuoto 5     //per la stampa ascii
...
template <typename T>
T MinHeap<T>::sumNode(int i, T x) {
    if(i >= heapSize || this->tree->at(i) >= x)
        return 0;
    T currentSum = this->tree->at(i);

    T leftSum = sumNode(left(i), x);
    T rightSum = sumNode(right(i), x);
